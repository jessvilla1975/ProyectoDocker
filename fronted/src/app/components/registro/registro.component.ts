...
export class RegistroComponent implements OnInit {
  ...
  passwordMatchValidator(form: FormGroup) {
    return form.get('contrasena')?.value === form.get('confirmarContraseña')?.value ? null : { mismatch: true };
  }
  ...
  onSubmit() {
    if (this.agregarUsuario.valid) {
        ...
        if (this.tipoUsuario === 'conductor') {
            ...
            const {
                id, genero, nombre, apellido, correo, telefono, direccion, fecha_nacimiento, contrasena,
